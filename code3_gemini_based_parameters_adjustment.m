...
%% Stage 2
function [pop, pcrossover_set_LLM, pmutation_set_LLM] = code3_gemini_based_parameters_adjustment(location_origin_uav_transpose, location_users, ...
    ...
    gemini_api_key = "xxxxxxx";  % 在 Google AI Studio 获取的 Key
    ...
    for it = 1:MaxIt_local
        ...
        body = struct( ...
            "contents", [ ...
                struct("parts", [ struct("text", prompt_text) ]) ...
             ] ...
        );
        
        headers = [ ...
            HeaderField("Content-Type", "application/json"), ...
            HeaderField("x-goog-api-key", gemini_api_key) ...
        ];

        options = matlab.net.http.HTTPOptions('ConnectTimeout', 60, 'ResponseTimeout', 60);
        request = RequestMessage('post', headers, body);

        try
            response = request.send(api_endpoint, options);
        catch ME
            warning('HTTP request to Gemini failed: %s', ME.message);
            continue;
        end
                
        respData = response.Body.Data;
        if isfield(respData, "candidates")  % 根据返回结构调整
            response_text = respData.candidates(1).content;
            disp("=== Response of Gemini ===");
            disp(response_text.parts);
        else
            disp('Error: Unexpected Gemini response format:');
            disp(respData);
        end
        ...
        % 执行正则匹配
        response_text = string(respData.candidates(1).content.parts(1).text);
