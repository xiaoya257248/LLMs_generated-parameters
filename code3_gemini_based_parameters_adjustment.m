...
function [pop, pcrossover_set_LLM, pmutation_set_LLM, time_LLM_pure] = stage_2_NSGA_II_Qwen(location_origin_uav_transpose, location_users, ...
    ...
    for it = 1:MaxIt_local
        ...
        try
            ...
            if isfield(response_data, 'choices') && ~isempty(response_data.choices)
                ...
                warning('Error: Could not find "choices" field. Full response data:');
                disp(response.Body.Data);
            end
        catch ME
