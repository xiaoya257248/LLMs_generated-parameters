...
function [pop, pcrossover_set_LLM, pmutation_set_LLM, time_LLM_pure] = stage_2_NSGA_II_ChatGPT(location_origin_uav_transpose, location_users, ...
    ...
    for it = 1:MaxIt_local        
        ...
        try
            ...
            if isstruct(response_data) && isfield(response_data, 'choices') && numel(response_data.choices) >= 1
                response_text = response_data.choices(1).message.content;
                disp(response.Body.Data);
                disp(response_text);
            elseif isstruct(response_data) && isfield(response_data, 'error')
                warning('LLM API returned an error: %s', response_data.error.message);
            elseif ischar(response_data) || isstring(response_data)
                response_text = char(response_data);
            else
                warning('LLM response format is unexpected. Continuing with previous parameters.');
            end
        catch ME
        ...
        if ~isempty(match)
            ...
            if ~isnan(new_pC) && ~isnan(new_pM)
                ...
                warning('数值转换失败 (包含非数字字符)，保留上一次参数。');
            end
        else
            warning('未能按预期格式找到参数，请检查回复，保留上一次参数。');
        end
